...
| **Persona picker** | Press **`Ctrl+Shift+P`** to open a searchable persona palette and apply a chosen persona to a single dictation — handy for browser webmail and terminal-hosted coding agents that auto-detection can't identify. Personas need AI enhancement on; if it's off, the pick is inserted verbatim with a reminder to enable AI. |
...
- **Picker (`Ctrl+Shift+P`)** — opens a searchable palette of every persona, including a **Coding Agent** persona for turning loose speech into a precise implementation prompt. This is the way to reach browser webmail and terminal-hosted coding agents that process-name detection can't identify. Personas run only with AI enhancement on; if it's off, the pick is inserted verbatim with a reminder to enable AI.
