...
public sealed class ControlPanelViewModel : ObservableObject
{
    ...
    /// Async here (unlike WPF's synchronous MessageBox) because Avalonia dialogs are shown asynchronously.
    /// </summary>
    public Func<ModelChangePrompt, Task<bool>>? ConfirmModelChange { get; set; }
    ...
    private async Task OnModelSelectedAsync(SpeechModelSize previous, SpeechModelSize target)
    {
        ...
        var proceed = ConfirmModelChange is null
            || await ConfirmModelChange(new ModelChangePrompt(target, installed, installed ? 0 : _models.ApproximateSizeBytes(target)));
